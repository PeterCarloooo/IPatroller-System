...
function IncidentsReports() {
  ...
  // Print summary state
  const printSummaryRef = React.useRef();
  const handlePrintSummary = () => {
    const printContents = printSummaryRef.current.innerHTML;
    const win = window.open('', '', 'height=700,width=900');
    win.document.write('<html><head><title>Incidents Summary</title>');
      ...
      <div className="card shadow-sm border-0">
        <div className="card-body p-0">
          <div className="d-flex justify-content-end px-3 pt-3 pb-2">
            <button className="btn btn-outline-primary" onClick={handlePrintSummary}>
              <i className="fas fa-print me-2"></i> Print Incidents Summary
            </button>
          </div>
          {/* Hidden print summary */}
          <div ref={printSummaryRef} style={{ display: 'none' }}>
